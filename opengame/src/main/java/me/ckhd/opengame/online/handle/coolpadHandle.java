	...
	public void parseParamter(String code, HttpServletRequest request,
		...
		try {
			...
			if(respData.size() > 0){
				...
				onlinePay.setActualAmount(data.getInteger("money")+"");
...
//			if (SignHelper.verify(transdata, sign,onlinePay.getPayInfoConfig().getExInfoMap().get("pay_key").toString())) {
			if (CpTransSyncSignValid.validSign(transdata, sign,onlinePay.getPayInfoConfig().getExInfoMap().get("pay_key").toString())) {
