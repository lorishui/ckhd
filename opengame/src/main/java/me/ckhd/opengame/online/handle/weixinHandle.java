...
public class weixinHandle extends BaseHandle{
	...
	public String pay(OnlinePay onlinePay, JSONObject codeJson) {
		...
		map.put("notify_url", StringUtils.isBlank(codeJson.getString("notifyUrl"))?onlinePay.getPayInfoConfig().getNotifyUrl():codeJson.getShort("notifyUrl"));//接收微信支付异步通知回调地址
		...
		if( StringUtils.isNotBlank(content) ){
			...
			if ("SUCCESS".equals(payContent.get("return_code"))) {
				...
				result.put("errMsg","微信下单失败！");
			}
		}else{
			result.put("errMsg","微信下单超时或网络问题！");
		}
