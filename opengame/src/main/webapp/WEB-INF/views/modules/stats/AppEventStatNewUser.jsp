...
<head>
	...
	<script type="text/javascript">
		...
		/*	if(endDate.getTime() - startDate.getTime() > 6 * 24 * 3600 * 1000){
			...
			changeAPPID(true);
			_w_table_rowspan("#statsEventTable",1);
			$("#searchForm").attr("action","${ctx}/stats/appEventStat/newUser");
			$("#searchForm").submit();
			return false;
		} 
...
<body>
	...
	<form:form id="searchForm"  modelAttribute="appEventStat" action="${ctx}/stats/appEventStat/newUser" method="post" class="breadcrumb form-search">
	<br>
		<div>
			<label>游戏：</label>
	   	...
	   	<div>
			...
			&nbsp;&nbsp;
			<span><font color="red">*</font> </span>
			<!--input id="endDate" name="endDate" type="text" readonly="readonly" maxlength="20" value=""
			class="WdateTime required" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/-->
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<table id="statsEventTable" class="table2excel table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th onclick="$.sortTable.sort('statsEventTable',1,compareNumber)" class="sort-column">渠道编号</th>
				<th onclick="$.sortTable.sort('statsEventTable',2,compareNumber)" class="sort-column">渠道名称</th>
				<th onclick="$.sortTable.sort('statsEventTable',3,compareNumber)" class="sort-column">新增</th>
				<th onclick="$.sortTable.sort('statsEventTable',4,compareNumber)" class="sort-column">活跃</th>
			</tr>
		 </thead>	
		 <tbody>
		 <c:forEach items="${statsList}" var="stats" varStatus="status">
				<tr>
					<td>${status.count} </td>
					<td>${stats.channelid}</td>
					<td>${fns:findChannelName(stats.channelid,"-")}</td>
					<td><c:choose><c:when test="${not empty stats.newUserNum}">
						${stats.newUserNum}
					</c:when><c:otherwise>0</c:otherwise></c:choose>
					</td>
					<td><c:choose><c:when test="${not empty stats.activeUserNum}">
						${stats.activeUserNum}
