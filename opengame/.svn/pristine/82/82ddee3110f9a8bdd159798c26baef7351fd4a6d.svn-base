	...
	<table id="paycodeTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${onlinePaycodeList}" var="onlinePaycode" varStatus="status">
				<tr>
					<td>${status.count}</td>
					<td>${fns:getByCkAppName(onlinePaycode.ckAppId)}(${onlinePaycode.ckAppId})</td>
					<td>${onlinePaycode.paycode_name}</td>
					<td>${onlinePaycode.total_num}</td>
					<td><fmt:formatNumber type="percent" value="${onlinePaycode.total_num/total_num}" maxFractionDigits="2" pattern="0.00%"/></td>
					<td>${onlinePaycode.succ_num}</td>
					<td><fmt:formatNumber type="percent" value="${onlinePayList[onlinePayList.size()-1].succ_num ne 0?0.1*(onlinePaycode.succ_num)/succ_num*10:0}" maxFractionDigits="2" pattern="0.00%"/></td>
					<td><fmt:formatNumber type="percent" value="${0.1*onlinePaycode.succ_num/onlinePaycode.total_num*10}" maxFractionDigits="2" pattern="0.00%"/></td>
					...
					<td><c:choose><c:when test="${not empty onlinePaycode.actualAmount && not empty onlinePayList[onlinePayList.size()-1].actualAmount}}">
						<fmt:formatNumber type="percent" value="${onlinePayList[onlinePayList.size()-1].actualAmount/(onlinePaycode.actualAmount)}" maxFractionDigits="2" pattern="0.00%"/>
					...
					<td><fmt:formatNumber type="percent" value="${onlinePayList[onlinePayList.size()-1].succ_actual ne '0.0000'?0.1*onlinePaycode.succ_actual/succ_actual*10:0}" maxFractionDigits="2" pattern="0.00%"/></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<table id="channelInfoTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${onlineChannelList}" var="stats" varStatus="status">
				<tr>
					<td>${status.count}</td>
					<td>${fns:getByCkAppName(stats.ckAppId)}(${stats.ckAppId})</td>
					...
					<fmt:formatNumber type="percent" value="${stats.succ_actual/stats.actualAmount}" maxFractionDigits="2" pattern="0.00%"/>
	...
	<tbody>
		<c:forEach items="${onlineChannelPaycodeList}" var="stats" varStatus="status">
			<tr>
				...
				<td><fmt:formatNumber type="percent" value="${stats.total_num/total_num}" maxFractionDigits="2" pattern="0.00%"/></td>
				<td>${stats.succ_num}</td>
				<td><fmt:formatNumber type="percent" value="${onlinePayList[onlinePayList.size()-1].succ_num ne 0?stats.succ_num/onlinePayList[onlinePayList.size()-1].succ_num:0}" maxFractionDigits="2" pattern="0.00%"/></td>
				<td><fmt:formatNumber type="percent" value="${stats.succ_num/(stats.total_num)}" maxFractionDigits="2" pattern="0.00%"/></td>
				<td><c:choose><c:when test="${not empty stats.actualAmount}">
					<fmt:formatNumber type="number" value="${stats.actualAmount}" maxFractionDigits="2" pattern="0.00"/></c:when>
					<c:otherwise>0.00</c:otherwise></c:choose></td>
				<td><c:choose><c:when test="${not empty actualAmount && not empty onlinePayList[onlinePayList.size()-1].actualAmount}">
					<fmt:formatNumber type="percent" value="${stats.actualAmount/actualAmount}" maxFractionDigits="2" pattern="0.00%"/></c:when>
					<c:otherwise>0.00%</c:otherwise></c:choose>
				</td>
				<td><fmt:formatNumber type="number" value="${stats.succ_actual}" maxFractionDigits="2" pattern="0.00"/></td>
				<td><fmt:formatNumber type="percent" value="${onlinePayList[onlinePayList.size()-1].succ_actual ne '0.0000'?stats.succ_actual/succ_actual:0}" maxFractionDigits="2" pattern="0.00%"/></td>
			</tr>
		</c:forEach>
	</tbody>
</table>
