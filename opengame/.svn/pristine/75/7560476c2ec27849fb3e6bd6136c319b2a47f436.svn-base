...
<script type="text/javascript">
	...
	function changeAPPID(pageLoad, update) {
		$.ajax({
			...
			success : function(data) {
				...
				if (pageLoad) {
					$("#carrierAppId").val("${mmapporder.appId}");
	...
	function chanageCarrierAppId() {
		var carrierAppId = "${payInfoConfig.carrierAppId}"
		...
		$("#carrierAppId option").each(
				function() {
					if ($(this).val() == carrierAppId) {
						$(this).attr("selected", true);
	...
	<form:form id="searchForm" modelAttribute="payInfoConfig"
		...
		<ul class="ul-form">
			<li><label class="control-label">游戏：</label> <form:select
				...
				</form:select> <form:select id="carrierAppId" path="carrierAppId"
	...
	<table id="contentTable"
		...
		<thead>
			<tr>
				...
				<th class="sort-column carrierAppId">MM游戏名称</th>
		...
		<tbody>
			<c:forEach items="${page.list}" var="payInfoConfig">
				<tr>
					...
					<td style="width: 10%">${payInfoConfig.appname }</td>
