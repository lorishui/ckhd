...
public class SyncAppOrderReq implements Serializable {
	...
	private Device send_Address = new Device();//发送方地址

	private Device dest_Address = new Device();//接收方地址
	...
	@XmlElement(name = "TransactionID", namespace = "http://www.monternet.com/dsmp/schemas/")
	...
	@XmlElement(name = "OrderPayment", namespace = "http://www.monternet.com/dsmp/schemas/", required = true)
	...
	@XmlElement(name = "MD5Sign", namespace = "http://www.monternet.com/dsmp/schemas/", required = true)
	...
	@XmlElement(name = "ReturnStatus", namespace = "http://www.monternet.com/dsmp/schemas/")
	...
	@XmlElement(name = "ProvinceID", namespace = "http://www.monternet.com/dsmp/schemas/")
	 ...
	 * @return
	 */
	public MmAppOrder genEntity(){
		MmAppOrder mmAppOrder = new MmAppOrder();
		mmAppOrder.setActionId(this.actionID);
		mmAppOrder.setActionTime(this.actionTime);
		mmAppOrder.setAppId(this.appID);
		
		mmAppOrder.setChannelId(this.channelID);
		mmAppOrder.setCheckId(this.checkID);
		// 由于字段太多，将所有set方法列出，不需要赋值的注释。下同。请保留不要删除。
		// mmAppOrder.setCreateBy(createBy);
		// mmAppOrder.setCreateDate(createDate);
		// mmAppOrder.setCurrentUser(currentUser);

		// mmAppOrder.setDelFlag(delFlag);
		mmAppOrder.setDestDeviceId(this.getDest_Address().getDeviceID());
		mmAppOrder.setDestDeviceType(this.getDest_Address().getDeviceType());
		
		mmAppOrder.setExtendData(this.exData);
		
		mmAppOrder.setFeeMsisdn(this.feeMSISDN);
		
		mmAppOrder.setMD5sign(this.mD5Sign);
		mmAppOrder.setMsgType(this.msgType);
		mmAppOrder.setMsisdn(this.mSISDN);
		
		mmAppOrder.setOrderId(this.orderID);
		mmAppOrder.setOrderPayment(this.orderPayment);
		mmAppOrder.setOrderType(this.orderType);
		
		// mmAppOrder.setPage(page);
		mmAppOrder.setPaycode(this.payCode);
		mmAppOrder.setPrice(this.price);
		mmAppOrder.setProgramId(this.programID);
		mmAppOrder.setProvinceID(this.provinceID);
		
		mmAppOrder.setReturnStatus(returnStatus);
		
		mmAppOrder.setSendDeviceId(this.getSend_Address().getDeviceID());
		mmAppOrder.setSendDeviceType(this.getSend_Address().getDeviceType());
		// mmAppOrder.setSqlMap(sqlMap);
		mmAppOrder.setSubsNumber(this.subsNumb);
		mmAppOrder.setSubsSeq(this.subsSeq);
		
		mmAppOrder.setTotalPrice(this.totalPrice);
		mmAppOrder.setTradeId(this.tradeID);
		mmAppOrder.setTransactionId(this.transactionId);

		// mmAppOrder.setUpdateBy(updateBy);
		// mmAppOrder.setUpdateDate(updateDate);
		mmAppOrder.setVersion(this.version);
