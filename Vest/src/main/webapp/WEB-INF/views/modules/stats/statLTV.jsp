		...
		<tbody>
			<c:forEach items="${data}" var="stat" varStatus="status">
				<tr>
					...
					<td>${stat.actNum }</td>
					<c:choose>
						<c:when test="${stat.newNum == 0}"><td>--</td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.actNum/stat.newNum}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv0 == -1 || stat.newNum == 0}"><td></td></c:when>
						<c:otherwise>
							<c:choose>
								<c:when test="${stat.timeframes eq currentDate}">
									<td><span style="color: #B8860B"><fmt:formatNumber value="${stat.ltv0*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></span></td>
								</c:when>
								<c:otherwise>
									<td><fmt:formatNumber value="${stat.ltv0*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td>
								</c:otherwise>
							</c:choose>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv1 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv1*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv2 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv2*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv3 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv3*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv4 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv4*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv5 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv5*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv6 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv6*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv7 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv7*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv14 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv14*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv30 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv30*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv60 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv60*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${stat.ltv90 == -1 || stat.actNum == 0}"><td></td></c:when>
						<c:otherwise><td><fmt:formatNumber value="${stat.ltv90*0.01/(stat.newNum+0.00001)}" pattern="0.00"/></td></c:otherwise>
					</c:choose>
				</tr>
			</c:forEach>
		</tbody>
