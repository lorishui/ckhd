...
<body> 
	<form:form id="searchForm"  modelAttribute="statRelate" action="${ctx}/stats/graphNew" method="post" class="breadcrumb form-search">
		...
		<div class="query_column_space">
			...
			<input id="startTime" name="startTime" type="text" readonly="readonly" maxlength="20" class="WdateTime required"
				value="${statRelate.startTime}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>到
			<input id="endTime" name="endTime" type="text" readonly="readonly" maxlength="20" class="WdateTime required"
				value="${statRelate.endTime}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
		</div>
		<div class="query_column_space">
			<label>统计时间精度选择：</label>
			<%-- <input type="radio" name="timeStyle" value="1" ${statRelate.timeStyle == 1 or statRelate.timeStyle == 0?'checked="checked"':'' }>月 --%>
			<input type="radio" name="timeStyle" value="2" ${statRelate.timeStyle == 2 ?'checked="checked"':'' }>日
			<input type="radio" name="timeStyle" value="1" ${statRelate.timeStyle == 1 ?'checked="checked"':''}>时
			&nbsp;&nbsp;&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>&nbsp;&nbsp;
		</div>
	</form:form>
	
	<div class="graph">
		<div class="position"><font size="3">设备新增统计</font></div>
	    <div class="bottom">
	    	<button value="新增设备" class="button">
	    		<label>
	    			<font size="1">新增设备</font><br/>
	    			<font size="5">${statRelate.newNum}</font><br/>
	    		</label>
	    	</button>
	    </div>
