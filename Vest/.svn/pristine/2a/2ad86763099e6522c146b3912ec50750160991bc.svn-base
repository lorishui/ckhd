	...
	public String graphicalMoney(StatMoney statMoney,Model model){
		...
		try{
			...
			if( !channelPermission.isEmpty() ) {
				statMoney.setPermissionChannel(StringUtils.join(channelPermission, ","));
			}
			Set<String> gamePermission = UserUtils.getGamePermission();
			if( !gamePermission.isEmpty() ) {
				statMoney.setPermissionCkAppId(StringUtils.join(gamePermission, ","));
			}
			Set<String> gameChildPermission = UserUtils.getGameChildPermission();
			if( !gameChildPermission.isEmpty() ) {
				statMoney.setPermissionCkAppChildId(new ArrayList<String>(gameChildPermission));
			}
			...
			list = this.statMoneyService.statsSuccessMoney(statMoney);
			param.setSuccessMoney(list.isEmpty() || list.get(0) == null ? 0 : list.get(0).getSuccessMoney()/100);
			param.setMoney(list.isEmpty() || list.get(0) == null ? 0 : list.get(0).getMoney()/100);
