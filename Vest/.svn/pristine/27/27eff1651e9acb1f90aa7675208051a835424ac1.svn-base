...
import me.ckhd.opengame.online.handle.common.aibei.SignHelper;
import me.ckhd.opengame.online.handle.common.lenovo.HttpUtils;
...
@Component("aibei")
@Scope("prototype")
public class aibeiHandle extends BaseHandle{
	...
	public String login(OnlineUser onlineUser, JSONObject codeJson,
		...
		String reqStr = reqData(payInfo.getAppid(), verifyInfo.getString("loginToken"),payInfo.getAppkey());
		log.info("登陆时发送给渠道的数据为:"+reqStr);
		String respStr = HttpUtils.sentPost(GOLOGINURL, reqStr,"UTF-8"); // 请求验证服务端
		if( StringUtils.isNotBlank(respStr) ){
			try {
				JSONObject json = split(respStr);
				if( json.size() > 1 ){
					JSONObject tran = JSONObject.parseObject(json.getString("transdata"));
					onlineUser.setSid(tran.getString("userId"));
					returnLoginSuccess(result, onlineUser);
				}else{
					result.put("errMsg", "用户验证失败！");
				}
			} catch (Exception e) {
				log.error("aibei login 数据解析异常！", e);
				result.put("errMsg", "aibei login 数据解析异常！");
			}
		}else{
	...
	private JSONObject split(String data) throws Exception{
...
//		map.put("price",Double.valueOf(onlinePay.getPayCodeConfig().getPrice())/100);
		...
		String reqStr = reqData(map);
		log.info("支付时发送给渠道的数据为:"+reqStr);
		String respStr = HttpUtils.sentPost(GOPAYURL, reqStr,"UTF-8"); // 请求验证服务端
		try {
			JSONObject json = split(respStr);
			JSONObject tran = JSONObject.parseObject(json.getString("transdata"));
			if( json.containsKey("sign") ){
				if (SignHelper.verify(json.getString("transdata"), json.getString("sign"),onlinePay.getPayInfoConfig().getExInfoMap().get("PLATP_KEY").toString())) {
					result.put("resultCode",0);
					result.put("errMsg","SUCCESS");
					JSONObject data = new JSONObject();
					data.put("transid", tran.getString("transid"));
				...
				result.put("resultCode", json.getInteger("code"));
				result.put("errMsg", tran.getString("errmsg"));
			}
		} catch (Exception e) {
			log.error("aibei login 数据解析异常！", e);
			result.put("resultCode",411);
	...
	public String reqData(Map<String, Object> params) {
		JSONObject jsonObject = new JSONObject();
	...
	public void parseParamter(String code, HttpServletRequest request,
		...
		try {
			...
			log.error("aibei callback 解析数据失败!", e);
		}
	}
