...
<script type="text/javascript">
	function page(n, s) {
		...
		if (s)
			$("#pageSize").val(s);
		$("#searchForm").attr("action", "${ctx}/app/adStatistics/findOperatorStaList");
		$("#searchForm").submit();
		return false;
	}
...
<body>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/app/adStatistics/findOperatorStaList">统计列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="adQueryEntity"
		action="${ctx}/app/adStatistics/findOperatorStaList" method="post"
		...
		<sys:tableSort id="orderBy" name="orderBy" value="${page.orderBy}"
			callback="page();" />
		<shiro:hasPermission name="app:adStatistics:admin">
			<label>负责人:</label>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		<thead>
			<tr>
				<th onclick="$.sortTable.sort('contentTable',0,compareNumber)" style="cursor: pointer;">运营人员</th>
