	...
	$(document).ready(function() {
		...
		$("#searchForm").validate({
			submitHandler : function(form) {
				$("#searchForm").attr("action", "${ctx}/stats/new");
				form.submit();
				merge();
			},
...
<body>
	...
	<form:form id="searchForm" modelAttribute="statRelate"
		action="${ctx}/stats/new" method="post" class="breadcrumb form-search">
			...
			<tr>
				...
				<c:choose>
				   <c:when test="${statRelate.groupChildCkAppId == 1}"> 
				    <th onclick="$.sortTable.sort('statsOnline',3,compareNumber)" style="cursor: pointer;">子游戏名称</th>      
				   </c:when>
				</c:choose>
				<c:choose>
