...
public class StatRelateController extends BaseController {
	...
	@RequestMapping("new")
	public String statNew(StatRelated statRelated,Model model,HttpSession session){
	...
	public String graphicalNew(StatRelated statRelated,Model model,HttpSession session){
		if (!Verdict.isAllow(statRelated.getCkAppId())) {
			return "403";
		}
		...
		try{
			...
			list = this.statRelateService.statsNew(statRelated);
			param.setNewNum(list.isEmpty() || list.get(0) == null ? 0 : list.get(0).getNewNum());
	...
	public String graphicalAct(StatRelated statRelated,Model model,HttpSession session){
		if (!Verdict.isAllow(statRelated.getCkAppId())) {
			return "403";
		}
		StringBuffer title = new StringBuffer();
		...
		try{
			...
			statRelated.setGroupBy("");
			list = this.statRelateService.statsAct(statRelated);
			param.setActNum(list.isEmpty() || list.get(0) == null ? 0 : list.get(0).getActNum());
	...
	private void getPermission(StatRelated statRelated, HttpSession session) {
		...
		//子游戏权限
		String skey = "ckappChildIdPermission";
		if( session.getAttribute(skey) == null ){
			session.setAttribute(skey, new ArrayList<String>(UserUtils.getGameChildPermission()));
		}
		statRelated.setPermissionCkAppChildId((List<String>) session.getAttribute(skey));
	}
		...
		for (StatRelated act : actlist) {
			for (StatRetention reten : retentionlist) {
				...
				if(act.getTimeframes().equals(reten.getRegTime())
						...
						act.setLtv60(reten.getD60Num());
						act.setLtv90(reten.getD90Num());
