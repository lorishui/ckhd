...
public class AppPayListController extends  BaseController{
	...
	public String save(AppPayList appPayList, HttpServletRequest request, Model model, 
		...
		if(StringUtils.isBlank(appPayList.getChildCkAppId())){
			appPayList.setChildCkAppId("1");
		}
		// 保存 
	...
	public  String delete(AppPayList appPayList , RedirectAttributes redirectAttributes){
		if (!Verdict.isAllow(appPayList.getCkAppId())) {
			return "403";
		}
		appPayListService.delete(appPayList);
		addMessage(redirectAttributes, "删除APPPLSIT成功");
		return  "redirect:" + adminPath + "/app/appPayList/list?repage";
