...
public class hailanHandle extends BaseHandle{
	 ...
	 */
	private Result<JsonObject> pay0(String uid, OnlinePay onlinePay, JSONObject codeJson) throws EncoderException, UnsupportedEncodingException {

		String payUrl = Global.getConfig("hailan.pay_url", "https://api.rightyoo.net/v1/?service=Pay.Unifiedorder");
		
		int connTimeout = Global.getConfig("hailan.connect_timeout", 1000 * 5);
		int readTimeout = Global.getConfig("hailan.request_timeout", 1000 * 5);
		
		String appName = Global.getConfig("hailan.app_name", "");
		String packageName = Global.getConfig("hailan.package_name", "");
		//String bundleId = Global.getConfig("hailan.bundle_id", "");
		...
		String sceneInfo = String.format("app_name=%s&package_name=%s", appName, packageName);
		...
		log.info("[{}]request:{},{}", new Object[]{uid, payUrl, request});
		...
		if( json.get("ret") != null && json.get("ret").getAsInt() == 200 ) {
			result.setCode(0);
	...
	private Result<Void> queryOrderState0(String uid, OnlinePay onlinePay,JSONObject codeJson) throws UnsupportedEncodingException {

		String url = Global.getConfig("hailan.query_url", "https://api.rightyoo.net/v1/?service=Pay.OrderQuery");
		...
		log.info("[{}]request:{},{}", new Object[]{uid, url, request});
	...
	public static void main(String argc[]) {
		...
		onlinePay.setNotifyUrl("http://i16785s665.iok.la/netpay/callback/rightyoo/1.1.0");
