...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/ad/adResource/list">ad资源列表</a></li>
		<li class="active"><a href="${ctx}/ad/adResource/form?id=${adResource.id}">ad资源<shiro:hasPermission name="ad:adResource:edit">${not empty adResource.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="ad:adResource:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="adResource" action="${ctx}/ad/adResource/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>
		<div class="control-group">
			<label class="control-label">APP:</label>
			<div class="controls">
				 <form:select path="ckAppId"  disabled="${not empty adResource.ckAppId?true:false}"  class="input-medium">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				value="${adResource.endDate}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
